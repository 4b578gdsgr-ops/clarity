...
export function validateBooking({ name, phone, email, contact_preference, address, issues, bikes, bike_details }) {
  ...
  // At least one issue or description per first item
  // 'other' type items use otherDescription instead of issues chips
  const effectiveIssues = bikes?.length > 0
    ? bikes.flatMap(b => b.type === 'box_ship'
        ? ((b.destination?.trim() && b.bikeDetails?.trim()) ? ['box_ship'] : [])
        : b.type === 'other'
          ? (b.otherDescription?.trim() ? ['other'] : [])
          : (b.issues || []))
    : issues;
