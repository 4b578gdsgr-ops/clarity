...
export async function runCleanup() {
  ...
  if (!bookings || bookings.length === 0) {
    console.log('[cleanupBookings] nothing to clean, cutoff=' + cutoff);
    return 0;
  }

  const bookingIds = bookings.map(b => b.id);
  console.log('[cleanupBookings] cutoff=' + cutoff, 'booking IDs eligible for media purge:', bookingIds);
  ...
  console.log('[cleanupBookings] storage paths to delete (' + photoPaths.length + '):', photoPaths);

  // Delete storage objects (ignore errors — files may already be gone)
  if (photoPaths.length > 0) {
    const { error: removeErr } = await supabaseAdmin.storage.from('booking-photos').remove(photoPaths);
    if (removeErr) console.error('[cleanupBookings] storage remove error:', removeErr.message);
  }
  ...
  console.log('[cleanupBookings] done. bookings marked media_purged_at (rows NOT deleted):', bookingIds, 'inspection reports with photo fields stripped:', (inspections || []).filter(i => (i.items || []).some(it => it.photo)).map(i => i.id));
