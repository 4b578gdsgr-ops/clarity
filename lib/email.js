...
export async function sendNewBookingNotification(booking) {
  console.log('[email] sendNewBookingNotification — booking id:', booking?.id, '| resend ready:', !!resend, '| KEY present:', !!process.env.RESEND_API_KEY, '| to:', ADMIN_EMAIL);
  console.log('[email] sendNewBookingNotification — full booking:', JSON.stringify(booking));
  ...
  const lines = [
    ...
    `Issues: ${issues}`,
    `Preferred: ${preferred}`,
    `Contact via: ${booking.contact_preference || 'not specified'}`,
  ...
  lines.push('', `View in admin: ${BASE_URL}/admin/service`);

  const subject = `New booking -- ${name} (${bike})`;
  const text = lines.join('\n');

  console.log('[email] sendNewBookingNotification — subject:', subject, '| text length:', text.length);
  try {
    const result = await resend.emails.send({ from: FROM, to: [ADMIN_EMAIL], subject, text });
    console.log('[email] sendNewBookingNotification — Resend result:', JSON.stringify(result));
    if (result?.error) {
      console.error('[email] sendNewBookingNotification — Resend returned error:', JSON.stringify(result.error));
    }
  } catch (err) {
    console.error('[email] sendNewBookingNotification — threw:', JSON.stringify(err), '| message:', err?.message, '| stack:', err?.stack);
  }
}
