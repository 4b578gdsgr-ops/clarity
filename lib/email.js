...
console.log('[email] resend client initialized:', !!resend, '| KEY present:', !!process.env.RESEND_API_KEY);
...
export async function sendServiceEmail(trigger, booking) {
  console.log('[email] sendServiceEmail called — trigger:', trigger, '| to:', booking?.email || '(no email)', '| resend ready:', !!resend);
  if (!resend) { console.error('[email] RESEND_API_KEY missing — skipping'); return; }
  if (!booking.email) { console.warn('[email] booking has no email — skipping'); return; }
...
export async function sendNewBookingNotification(booking) {
  console.log('[email] sendNewBookingNotification — booking id:', booking?.id, '| resend ready:', !!resend);
  if (!resend) { console.error('[email] RESEND_API_KEY missing — skipping new booking notification'); return; }
  ...
  lines.push('', `View in admin: ${BASE_URL}/admin/service`, '', '— Love Over Money');
