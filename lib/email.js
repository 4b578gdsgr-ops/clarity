...
/**
 * Send the Google review ask after a completed booking is marked paid.
 * Only call this when contact_preference === 'email'.
 */
export async function sendReviewRequestEmail(booking) {
  console.log('[email] sendReviewRequestEmail — to:', booking?.email || '(no email)', '| resend ready:', !!resend);
  if (!resend) { console.error('[email] RESEND_API_KEY missing — skipping review request email'); return; }
  if (!booking.email) { console.warn('[email] booking has no email — skipping review request email'); return; }
  ...
  try {
    await resend.emails.send({
      from: FROM,
      to: [booking.email],
      subject: 'Thanks for riding with us — One Love',
      text: [
        `Hey ${name},`,
        '',
        "Thanks for trusting us with your bike. We hope it rides like new.",
        '',
        `If you've got 30 seconds, a Google review would really help other riders find us:`,
        GOOGLE_REVIEW_URL,
        '',
        "No pressure either way. We're here whenever you need us.",
        '',
        '— One Love Outdoors',
      ].join('\n'),
    });
    console.log('[email] Review request email sent to', booking.email);
  } catch (err) {
    console.error('[email] Failed to send review request email to', booking.email, ':', err?.message || err);
  }
}
...
/**
 * Notify admin of a new one-box service inquiry.
 * No inner try/catch — errors propagate so they're visible in logs.
 */
export async function sendNewInquiryAdminEmail(inquiry) {
  console.log('[email] sendNewInquiryAdminEmail — resend ready:', !!resend, '| inquiry id:', inquiry?.id);
  if (!resend) {
    throw new Error('[email] RESEND_API_KEY missing — cannot send inquiry notification');
  }
  const { data, error } = await resend.emails.send({
    from: FROM,
    to: [ADMIN_EMAIL],
    subject: `New service inquiry — ${inquiry.name || 'Unknown'}`,
    text: [
  ...
  console.log('[email] Admin inquiry email sent — resend id:', data?.id);
  return data;
}
