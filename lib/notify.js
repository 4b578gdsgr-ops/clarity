...
import { sendServiceEmail, sendMessageEmail } from './email';
...
function buildSMS(trigger, booking) {
  switch (trigger) {
    case 'new':
      return `One Love: Got your service request. We'll confirm a pickup time shortly. Track: ${trackingLink(booking.id)}`;
    ...
    case 'picked_up':
      return `One Love: Your bike is with us. We'll keep you posted.`;

    case 'ready': {
      const date = booking.return_date ? fmtDate(booking.return_date) : 'soon';
      return `One Love: Your bike is ready. Delivering ${date}.`;
    }
 ...
 * - contact_preference === 'text'  → SMS only
...
export async function notifyCustomer(trigger, booking) {
  ...
  if (pref === 'text') {
    if (!booking.phone) {
      console.warn('[notify] contact_preference=text but no phone number — skipping SMS for', trigger);
      return;
    }
    const smsBody = buildSMS(trigger, booking);
    if (!smsBody) return;
    try {
      await sendSMS(booking.phone, smsBody);
      console.log('[notify] SMS sent for', trigger, 'to', booking.phone);
    } catch (err) {
      console.error('[notify] SMS FAILED for', trigger, ':', err?.message || err);
    }
  } else {
    // email (default)
