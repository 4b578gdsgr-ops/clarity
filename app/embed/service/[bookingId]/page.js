  ...
  if (step === 'done') {
    return (
      <div style={{ textAlign: 'center', padding: '20px 0' }}>
        ...
        <a href="https://oneloveoutdoors.org/service" target="_blank" rel="noreferrer" style={{ fontSize: 14, color: '#1a3328', textDecoration: 'underline' }}>Book a new service →</a>
      </div>
    );
  }
...
export default function EmbedBookingStatusPage({ params }) {
  ...
  console.log('[embed/service render] status:', booking.status, '| showCancel:', ['new','confirmed'].includes(booking.status));
  ...
  return (
    <div style={containerStyle}>
        ...
        return (
          <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: 20, marginBottom: 16 }}>
            ...
            {hasData ? (
              <>
                {wearItems.length > 0 && (
                  <div style={{ marginBottom: replaced.length || done.length || sentOut.length || attention.length || adjusted.length || good.length ? 16 : 0 }}>
      ...
      {['new', 'confirmed'].includes(booking.status) && (
        <CancelSection booking={booking} bookingId={bookingId} onUpdated={loadData} onOpenMessages={() => {
          document.getElementById('embed-messages')?.scrollIntoView({ behavior: 'smooth' });
          document.getElementById('embed-message-input')?.focus();
        }} />
      )}
      ...
      <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 12, padding: 20, marginTop: 16, textAlign: 'center' }}>
