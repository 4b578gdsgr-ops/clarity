...
function DeliveryConfirmSection({ booking, bookingId, onUpdated }) {
  ...
  if (phase === 'confirmed') {
    // Admin-confirmed delivery_time takes priority over customer preference
    ...
    if (adminTime) {
      // Admin has set the actual delivery time — show it definitively
    ...
    // No admin time yet — show customer preference if available
...
// Address is locked once we have the bike (in_progress or later); contact info always editable
