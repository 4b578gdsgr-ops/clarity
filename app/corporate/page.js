...
export default function CorporatePage() {
  ...
  useEffect(() => {
    fetch('/api/request').then(r => r.json()).then(d => setCommunityCount(d.count)).catch(() => {});
  }, []);
    ...
    <div className="min-h-screen relative">
      ...
      <div className="relative z-10 px-4 py-8 max-w-2xl mx-auto">
        <div className={`text-center mb-8 transition-all duration-700 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-5'}`}>
          ...
          <p className="text-sm" style={{ color: '#9ca3af' }}>Search any company by name, ticker, or brand.</p>
        </div>
        ...
        {!result && !searching && (
          <div className={`mb-6 transition-opacity duration-700 delay-300 ${mounted ? 'opacity-100' : 'opacity-0'}`}>
            ...
            <div className="text-center mt-4 text-[11px]" style={{ color: '#9ca3af' }}>
              60+ companies · 150+ brands & subsidiaries searchable
            </div>
            {communityCount > 0 && (
        ...
        {result && result !== 'not_found' && (
          <CompanyResult data={result} onSearch={(name) => { setQuery(name); window.scrollTo({ top: 0, behavior: 'smooth' }); doSearch(name); }} />
        )}
          ...
          <div className="text-center py-10 max-w-sm mx-auto">
            ...
            {!requestSent ? (
              <div className="p-4 rounded-xl" style={{ background: '#ffffff', border: '1px solid #e5e0d8', boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={async () => {
                    ...
                    if (!name) return;
                    try {
                      const res = await fetch('/api/request', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ company: name }) });
                      const data = await res.json();
              ...
              <div className="p-4 rounded-xl" style={{ background: '#f6fbf8', border: '1px solid #d1ead9' }}>
                ...
                {requestNumber && <div className="text-base font-extrabold font-mono mb-1" style={{ color: '#d97706' }}>You're request #{requestNumber}</div>}
